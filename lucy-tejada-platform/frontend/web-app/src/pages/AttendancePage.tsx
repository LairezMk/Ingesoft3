  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={saveAttendance} className="btn-primary">Guardar Asistencia</button>
      </div>
      ...
      <div className="card p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <div className="relative">
              ...
              <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="input pl-10 w-full" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Grupo</label>
            <select value={selectedGroup} onChange={(e) => setSelectedGroup(e.target.value)} className="input w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="card p-4 bg-primary-50 dark:bg-primary-900/10">
            <p className="text-sm text-dark-600 dark:text-dark-400">Total Estudiantes</p>
          ...
          <div className="card p-4 bg-success-50 dark:bg-success-900/10">
            <p className="text-sm text-dark-600 dark:text-dark-400">Presentes</p>
            <p className="text-2xl font-bold text-success-600">{attendance.filter(s => s.present).length}</p>
          </div>
          <div className="card p-4 bg-error-50 dark:bg-error-900/10">
            <p className="text-sm text-dark-600 dark:text-dark-400">Ausentes</p>
            <p className="text-2xl font-bold text-error-600">{attendance.filter(s => !s.present).length}</p>
          </div>
        </div>

        <div className="space-y-2">
          {attendance.map(student => (
            <div key={student.id} className="flex items-center justify-between p-4 rounded-lg border border-dark-200 dark:border-dark-700 hover:bg-dark-50 dark:hover:bg-dark-800">
